import laptop from './images/laptop1.jpeg';
import mobile from './images/mobile1.jpeg';
import tab from './images/tab1.jpeg';
import shoe from './images/shoe1.png';
import watch from './images/watch1.jpeg';
import headset from './images/headset1.jpeg';
import ac from './images/ac1.jpg';
import tv from './images/tv1.jpeg';

import mobile1 from './images/mobile1.jpeg'
import mobile2 from './images/mobile2.jpeg'
import mobile3 from './images/mobile3.jpeg'
import mobile4 from './images/mobile4.jpeg'
import laptop1 from './images/laptop1.jpeg'
import laptop2 from './images/laptop2.jpeg'
import laptop3 from './images/laptop3.jpeg'
import laptop4 from './images/laptop4.jpeg'
import tab1 from './images/tab1.jpeg'
import tab2 from './images/tab2.jpeg'
import tab3 from './images/tab3.jpeg'
import tab4 from './images/tab4.jpeg'
import shoe1 from './images/shoe1.png'
import shoe2 from './images/shoe2.jpg'
import shoe3 from './images/shoe3.jpg'
import shoe4 from './images/shoe4.png'
import tv1 from './images/tv1.jpeg'
import tv2 from './images/tv2.jpeg'
import tv3 from './images/tv3.jpeg'
import tv4 from './images/tv4.jpeg'
import watch1 from './images/watch1.jpeg'
import watch2 from './images/watch2.jpeg'
import watch3 from './images/watch3.jpeg'
import watch4 from './images/watch4.jpeg'
import headset1 from './images/headset1.jpeg'
import headset2 from './images/headset2.jpeg'
import headset3 from './images/headset3.jpeg'
import headset4 from './images/headset4.jpeg'
import ac1 from './images/ac1.jpg'
import ac2 from './images/ac2.jpeg'
import ac3 from './images/ac3.jpeg'
import ac4 from './images/ac4.jpeg'

export const products = [
  {
    id: 1,
    name: "DELL Laptop",
    image: laptop
  },
  {
    id: 2,
    name: "Iphone 17 Pro",
    image: mobile
  },
  {
    id: 3,
    name: "Nike Shoe",
    image: shoe
  },
  {
    id: 4,
    name: "Titan Watch",
    image: watch
  },
  {
    id: 5,
    name: "Lenovo tab",
    image: tab
  },
  {
    id: 6,
    name: "BoAt Headset",
    image: headset
  },
  {
    id: 7,
    name: "Orient AC",
    image: ac
  },
  {
    id: 8,
    name: "Samsung TV",
    image: tv
  }
];



  export  const productsdata = [
  {
    id: 1,
    name: "DELL Inspiron 15",
    category: "Laptop",
    brand: "Dell",
    details:
      "Dell Inspiron 3530 Laptop - Intel Core i3-1305U Processor, 8GB DDR4 RAM, 512GB SSD, Windows 11 Home, Onsite Service - Carbon Black",
    specialFeature: "13th Gen Intel Core i3 Processor",
    rewardPoints: 120,
    price: "₹ 55,000.00",
    rating: 4.5,
    images: [laptop1, laptop2, laptop3, laptop4],
    quantity:1
  },

  {
    id: 2,
    name: "Iphone 17 Pro",
    category: "Mobile",
    brand: "Iphone",
    details:
      "Apple iPhone 17 Pro 256 GB: 15.93 cm (6.3″) Display 120Hz, Breakthrough Battery Life, Pro Fusion Camera System with Center Stage Front Camera; Silver",
    specialFeature: "200MP Ultra-Clear Camera",
    rewardPoints: 130,
    price: "₹ 1,30,000.00",
    rating: 4.9,
    images: [mobile1, mobile2, mobile3, mobile4],
    quantity:1
  },

  {
    id: 3,
    name: "Nike Shoe",
    category: "Footwear",
    brand: "Nike",
    details:
      "Nike Vomero Plus White / UK 9.5 Sneaker Comfort, Speed & Style Combined",
    specialFeature: "Ultra Cushion Running Comfort",
    rewardPoints: 50,
    price: "₹ 5,000.00",
    rating: 3.8,
    images: [shoe1, shoe2, shoe3, shoe4],
    quantity:1
  },

  {
    id: 4,
    name: "AVI-8 Watch",
    category: "Watch",
    brand: "AVI",
    details:
      "AVI-8 Raga Viva Quartz Analog with Date White Dial Leather Strap Watch for Men",
    specialFeature: "Elegant Leather Strap Design",
    rewardPoints: 15,
    price: "₹ 3,100.00",
    rating: 4.4,
    images: [watch1, watch2, watch3, watch4],
    quantity:1
  },

   {
    id: 5,
    name: "Lenovo tab",
    category: "tab",
    brand: "Lenovo",
    details:
      "lenovo tab",
    rewardPoints: 45,
    price: "₹ 3,100.00",
    rating: 4,
    images: [tab1,tab2,tab3,tab4],
    quantity:1
  },
  {
    id: 6,
    name: "boAt Rockerz",
    category: "Headphones",
    brand: "boAt",
    details:
      "boAt Rockerz 480, RGB LEDs, 6 Light Modes, 60H Battery, Wireless Over-Ear Headphones",
    specialFeature: "60-Hour Battery Backup",
    rewardPoints: 20,
    price: "₹ 1,600.00",
    rating: 4.3,
    images: [headset1, headset2, headset3, headset4],
    quantity:1
  },

  {
    id: 7,
    name: "Orient AC",
    category: "Air Conditioner",
    brand: "Orient",
    details:
      "Orient 1.5 Ton Inverter Split AC FB-18X (White) Smart Cooling System Project – Energy Efficient Climate Control",
    specialFeature: "Energy Efficient Inverter Cooling",
    rewardPoints: 80,
    price: "₹ 45,000.00",
    rating: 3.5,
    images: [ac1, ac2, ac3, ac4],
    quantity:1
  },

  {
    id: 8,
    name: "Samsung Smart TV",
    category: "Television",
    brand: "Samsung",
    details:
      "Samsung Vision AI Smart TV Q7F (2025) 50-inch QLED 4K Ultra HD Display Smart Entertainment with Next-Gen AI Features",
    specialFeature: "Vision AI QLED 4K Display",
    rewardPoints: 75,
    price: "₹ 60,000.00",
    rating: 4.8,
    images: [tv1, tv2, tv3, tv4],
    quantity:1
  },
];